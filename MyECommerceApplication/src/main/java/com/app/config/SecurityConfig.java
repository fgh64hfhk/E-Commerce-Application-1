	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
			...
			.authorizeHttpRequests((authorizeHttpRequests) -> authorizeHttpRequests
					.requestMatchers("/public/**").permitAll() // 允許對首頁，登入頁面和靜態資源（如CSS、JS、圖片）的訪問，無需認證
					.requestMatchers("/admin/**").hasRole("ADMIN") // 只有管理員角色可以訪問
					.requestMatchers("/user/**").hasAnyRole("USER", "ADMIN") // 使用者和管理員角色可以訪問的資源
					.anyRequest().authenticated()) // 要求所有其他請求都必須經過認證
			.formLogin(form -> form
					.loginPage("/login")
					.permitAll()) // 允許所有用戶訪問表單登錄頁面
			.logout(logout -> logout
					.logoutSuccessUrl("/public/index") // 登出成功後跳轉到首頁
